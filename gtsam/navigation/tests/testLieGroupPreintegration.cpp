...
TEST(LieGroupPreintegration, BiasCorrectionJacobians) {
  ...
  EXPECT(assert_equal(numericalDerivative21(deltaRij, kZero, kZero),
                      Matrix3(Z_3x3)));
  EXPECT(assert_equal(numericalDerivative22(deltaRij, kZero, kZero),
                      pim.delRdelBiasOmega(), 1e-3));

  EXPECT(assert_equal(numericalDerivative21(deltaPij, kZero, kZero),
                      pim.delPdelBiasAcc()));
  EXPECT(assert_equal(numericalDerivative22(deltaPij, kZero, kZero),
                      pim.delPdelBiasOmega(), 1e-3));

  EXPECT(assert_equal(numericalDerivative21(deltaVij, kZero, kZero),
                      pim.delVdelBiasAcc()));
  EXPECT(assert_equal(numericalDerivative22(deltaVij, kZero, kZero),
...
TEST(LieGroupPreintegration, computeError) {
  ...
  std::function<Vector9(const NavState&, const NavState&,
      ...
      f = std::bind(&LieGroupPreintegration::computeError, pim, _1, _2, _3,
